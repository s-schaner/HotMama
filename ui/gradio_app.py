...
class GradioVolleyApp:
    ...
    def _build_interface(self) -> gr.Blocks:
        with gr.Blocks(title="VolleySense") as demo:
            ...
            with gr.Tab("Heatmap"):
                heatmap_session = gr.Textbox(label="Session ID (optional)", placeholder="Outputs saved under this session when provided")

                gr.Markdown("### Video → Heatmap Pipeline")
                pipeline_video = gr.File(label="Video", file_types=["video"], type="filepath")
                pipeline_method = gr.Dropdown(
                    choices=self.detection_methods,
                    value=self.detection_methods[0] if self.detection_methods else "auto",
                    label="Detection method",
                )
                with gr.Row():
                    pipeline_stride = gr.Slider(1, 8, value=2, step=1, label="Frame stride")
                    pipeline_smoothing = gr.Slider(1, 15, value=3, step=1, label="Smoothing window")
                with gr.Row():
                    pipeline_min_conf = gr.Slider(0.0, 1.0, value=0.05, step=0.01, label="Min confidence")
                    pipeline_overlay = gr.Checkbox(value=True, label="Generate overlays")
                ...
                pipeline_gallery = gr.Gallery(label="Heatmap renders", show_label=True, columns=3)
    ...
    def _run_heatmap_pipeline(
        ...
        session_key = session_id.strip() or "adhoc"
        run_dir = self.sessions_dir / session_key / "heatmaps" / f"run_{uuid.uuid4().hex}"
        ...
        logs = "\n".join(artifacts.logs) if artifacts.logs else "Pipeline completed without warnings."
        return (
            status,
            gallery,
