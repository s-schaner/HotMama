...
| D16 | Vision inference tiers | The `vlm` worker engine speaks OpenAI-compatible vision chat (vLLM), configured per run via `--vlm-url/--vlm-model` or a tier config file. Current tiers on "corona" (DGX Spark; 10.187.0.27 via ZeroTier, 192.168.86.80 on the home LAN): `:8000` qwen3-vl-30b (262k ctx, deep), `:8005` qwen3-vl-8b (131k, standard), `:8004` qwen3-vl-4b (16k, fast triage) — see `examples/vlm-tiers.example.json`. **VLM observations are attribution-free** (near/far court, jerseys seen) until camera-side calibration exists; the model must not guess team mapping it cannot know. | 2026-08-13, tiers provided by Stefan. |
| D18 | Court calibration | Manual corner-tap homography first (per SPEC's fallback), auto-detection later. Two modes: `full` (four baseline corners) and `near_half` (near baseline + net corners) — validated on real tournament footage, where a typical tripod height compresses the entire far half into ~40 px, making far-baseline taps unreliable and `near_half` the recommended mode; far positions extrapolate through the same homography. Calibrations are saved per **venue** (SPEC: calibrate once a season), attached to sessions, and delivered to workers inside each lease. The detect engine maps **foot points** (bottom-center of boxes — the ground plane) into court meters; near/far stays camera-relative until side-mapping events exist. | 2026-08-14, validated on Stefan's clips. |
| D17 | Detection + tracking stack | `supervision` (Detections as the common currency; PolygonZone/ViewTransformer reserved for the calibration phase) + the `trackers` package's ByteTrack (supervision's own tracker is deprecated for removal). The `detect` worker engine runs any `FrameDetector` → ByteTrack → pixel-space `player_tracks` observations (persistent-track counts + center-density grid, the seed of heatmaps). Default detector is lazily-loaded ultralytics YOLO (`detect` extra) so torch never burdens the server or CI; tracker id `-1` is the unactivated-detection sentinel and never counts as a player. | 2026-08-13, supervision suggested by Stefan (also in the original SPEC tech table). |
